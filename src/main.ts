...
class GridItem {
  ...
  move() {
    ...
    const render = () => {
      ...
      configs.tx = gsap.utils.interpolate(configs.tx, gsap.utils.mapRange(0, win.width, -xstart, xstart, mousepos.x), 0.07)
      configs.ty = gsap.utils.interpolate(configs.ty, gsap.utils.mapRange(0, win.height, -ystart, ystart, mousepos.y), 0.07);
      ...
      .to(this.items,
        {
          ...
          stagger: { amount: 0.2, grid: 'auto', from: 'random', ease: 'power3.out' }
        }, 0);
  }
}
